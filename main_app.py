...
def execute_download(mese_nome, anno, user, pwd, is_13ma):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Campi login specifici (da DOM: name=username/password dentro #ParametriLogin)
            ...
            # Submit robusto: click su 'Accedi', fallback Enter
            submitted = False
            try:
            ...
            if not submitted:
                try:
                    page.locator("text=Accedi").first.click()
                    submitted = True
                except Exception:
                    pass
            ...
            # Verifica login: più segnali post-login (evita falsi negativi su un solo testo)
            login_ok = False
            for sel in [
                "text=I miei dati",
                "text=Time",
            ...
            if not login_ok:
                ...
                try:
                    ...
                    st.image(page.screenshot(full_page=True), caption="Debug post-login")
                except Exception:
                    pass
                browser.close()
                return results
