...
if "res" in st.session_state:
    ...
    with col1:
        netto = dg.get("netto", 0)
        st.markdown(
            f"""
            <div class='metric-card'>
                <div class='metric-value'>€{netto:,.2f}</div>
                <div class='metric-label'>💶 Netto Busta</div>
            </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with col2:
        giorni_pagati = int(dg.get("giorni_pagati", 0))
        st.markdown(
            f"""
            <div class='metric-card'>
                <div class='metric-value'>{giorni_pagati}</div>
                <div class='metric-label'>📅 Giorni Pagati</div>
            </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with col3:
        if c:
            giorni_lavorati = c.get("giorni_lavorati", 0)
            st.markdown(
                f"""
                <div class='metric-card'>
                    <div class='metric-value'>{giorni_lavorati if giorni_lavorati > 0 else "N/D"}</div>
                    <div class='metric-label'>⏱️ Giorni Lavorati</div>
                </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    with col4:
        eventi_tot = agenda.get("total_events", 0) if isinstance(agenda, dict) else 0
        st.markdown(
            f"""
            <div class='metric-card'>
                <div class='metric-value'>{eventi_tot}</div>
                <div class='metric-label'>🗓️ Eventi Agenda</div>
            </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    with tab2:
        if c:
            ...
            v_malattia = (
                final_malattia if "final_malattia" in locals() else c.get("malattia", 0)
            )
            v_omesse = (
                final_omesse
                if "final_omesse" in locals()
                else c.get("omesse_timbrature", 0)
            )
            v_permessi = c.get("permessi", 0)
            v_riposi = (
                final_riposi if "final_riposi" in locals() else c.get("riposi", 0)
            )
            ...
            k4.metric(
                "⚠️ Omesse",
                v_omesse,
                delta=d_omesse if d_omesse != 0 else None,
                delta_color="inverse",
            )
